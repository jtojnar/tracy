...
// enables .tracy-tabs, .tracy-tab-label, .tracy-tab-panel
class Tabs
{
	static init() {
		...
		document.querySelectorAll('.tracy-tabs').forEach((context) => {
			let bar = context.querySelector('.tracy-tab-label').parentNode;
			if (!bar.querySelector('.tracy-tab-label.tracy-active')) {
				context.querySelector('.tracy-tab-label').classList.add('tracy-active');
				context.querySelector('.tracy-tab-panel').classList.add('tracy-active');
			}
		});
